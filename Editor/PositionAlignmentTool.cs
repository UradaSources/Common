...
public class PositionAlignmentTool
{
	...
	private Rect SelectedTrListPanel(Vector2 pos, Color? c)
	{
		...
		if (GUILayout.Button("Update Selected"))
		{
			...
			m_selectedTrList.AddRange(MiscUtils.GetSelectedComponentsByOrder<Transform>());
		...
		if (GUILayout.Button("Append Selected"))
		{
			...
			MiscUtils.Intersection(ref m_selectedTrList, itor);

			this.TryUpdateBaseObject();
		}
		if (GUILayout.Button("Remove Selected"))
		{
			var go = MiscUtils.GetSelectedGameObjectInScene().GetEnumerator().Current;
